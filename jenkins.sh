...
if [[ $# -lt 1 ]] || [[ "$1" == "--"* ]]; then
  ...
  exec java -Dhudson.TcpSlaveAgentListener.hostName="$(curl http://169.254.169.254/latest/meta-data/local-ipv4)" -Duser.home="$JENKINS_HOME" "${java_opts_array[@]}" -jar ${JENKINS_WAR} "${jenkins_opts_array[@]}" "$@"
fi
